...
- **雙層 PIN（很多房間鎖 + 一把 Master Key）**：
  ...
  - **專案 PIN**：每個專案可設**多把**（各可加暱稱），只能管理**該專案**；預設不能動別人投稿的內容，主 PIN 可個別授權權限旗標：`edit_others`（改別人投稿）、`edit_points`（改定位點）、`delegate_admin`（可建立「管理PIN」型分享連結）。
  - 主後台可**新增/移除**各層 PIN（簡易權限管理，非帳號系統）。
- **分享編輯連結**：後台可產生一次性顯示的連結，授予對方投稿身分（私人PIN／僅限匿名）或管理PIN 身分，可選填到期時間／使用次數上限（只擋「新增投稿」或「新登入」，不影響已投稿內容的編輯/刪除）。秘密只透過網址 fragment（`#redeem=...`）傳遞，讀取後即清除，不落地在伺服器或瀏覽器紀錄。
